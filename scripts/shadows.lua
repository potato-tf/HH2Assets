...
local function cashforhits(activator)
	...
	callbacks.damagetype = activator:AddCallback(ON_DAMAGE_RECEIVED_PRE, function(_, damageInfo)
		...
		if InstakillDuration > 0 and activator.m_szNetname == "Zombie" and hitter:InCond(56) == 1 then --added here to fix instakill bug -washy
			damage = curHealth
...
function ActivateFireSale()
	for _, player in pairs(ents.GetAllPlayers()) do
		if player:IsRealPlayer() then
			...
			if FireSaleDuration <= 0 then
				timer.Simple(1,function() player:PlaySoundToSelf("shadows/gallery_music.mp3") end)
			end
		end
	end
